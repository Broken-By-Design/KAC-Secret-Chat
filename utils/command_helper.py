...
def parse_command(message: str, nickname: str, timestamp: str) -> None:
    ...
    if message.startswith("/joke"):
        try:
            ...
            globals.socketio.emit('chat_message', { 'message': f"I couldn't fetch a joke! check the server status: https://status.killallchickens.org/report/uptime/a03d13d0a05da5a94df473ae71f8d648/ techinal info {response}", 'nickname': "Joke-Bot", 'timestamp': timestamp, 'system': True })
            add_chatlog_entry(f"I couldn't fetch a joke! check the server status: https://status.killallchickens.org/report/uptime/a03d13d0a05da5a94df473ae71f8d648/ techinal info {response}", "Joke-Bot", timestamp, globals.current_log_file, type="system")
            return
